	...
	filepath.Walk(dir, func(path string, f os.FileInfo, err error) error {
		...
		if m, _ := regexp.MatchString("^\\S+\\.apk$",f.Name()); m{
			...
			for i := 0; i < len(serverlist) ;i++ {
				...
				go ftpUploadFile(serverlist[i], "apkUpload", "wirelessgroup", path, domainName,f.Name())
			}
			for i := 0; i < len(serverlist) ;i++   {
				<- quit
			}
			//ftpUploadFile("124.251.46.171:21", "apkUpload", "wirelessgroup", path, domainName,f.Name())
		}
		return nil
	})
